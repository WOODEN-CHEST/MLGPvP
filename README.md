...
The plugin offers ways to change the settings of a game. While they techincally *can* be changed mid-match, most of them will only have an effect if they are changed before you start the match.

Settings can be change with the `mlgpvp setting` command.
...
You can change a setting by typing `mlgpvp setting <setting_name> <setting_value>`. The tab autocompelte will suggest the default value for this setting, but you can put any value you want in there.
...
To solve this, there exist **confugrations**.
...
To load a confugration, type `mlgpvp config load "<config_name>"`.

Similarly, to delete a config, just type `mlgpvp config delete "<config_name>"`.

To list your available configrations, use `mlgpvp config list`.

The plugin comes with a few built-in confugrations which you can use. Those cannot be deleted, and you cannot save a custom config with the same name as a built-in config.
...
# Building the plugin.
To build the plguin, you'll firstly need to not only include all of PaperMC's libraries, but also include the library [PluginCommon](https://github.com/WOODEN-CHEST/PluginCommon).
